...
Check for correspondance of metadata and counts.
...
First task is to find which columns in **counts** correspond to the countrol conditions (i.e. no drug). Then we can calculate the mean count value for each gene.
...
Now do the dame thing for the drug treated samples (i.e. **treated** columns)
...
Combine the mean count values for control and treated into a new data frae called meancounts
...
Now we can plot the drug treated vs non-drug control count values. (If drug does nothing, we expect a straight line when plotting control vs treated.)
...
Put this on a log scale to help see the data better.
...
There are `sum(up.ind)` regulated genes and `sum(down.ind)` down regulated genes.
...
plot(res$log2FoldChange, -log(res$padj), col=mycols, ylab="-log10(p-value)", xlab="log2FoldChange")
```
